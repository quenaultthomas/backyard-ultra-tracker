...
const ByteMatrix = require('../../common/ByteMatrix.cjs');
...
function embedAlignmentPatterns(matrix, { version }) {
  if (version >= 2) {
    const { alignmentPatterns } = Version.VERSIONS[version - 1];
...
function makeFormatInfoBits(bits, ecLevel, mask) {
  ...
  const bchCode = utils.calculateBCHCode(formatInfo, FORMAT_INFO_POLY);
  bits.append(bchCode, 10);
  const maskBits = new BitArray.BitArray();
...
function embedFormatInfo(matrix, ecLevel, mask) {
  const formatInfoBits = new BitArray.BitArray();
...
function makeVersionInfoBits(bits, version) {
  ...
  const bchCode = utils.calculateBCHCode(version, VERSION_INFO_POLY);
...
function embedVersionInfo(matrix, { version }) {
  if (version >= 7) {
    const versionInfoBits = new BitArray.BitArray();
...
function embedCodewords(matrix, codewords, mask$1) {
  let bitIndex = 0;
  const { size } = matrix;
    ...
    for (let y = 0; y < size; y++) {
      for (let i = 0; i < 2; i++) {
        ...
        if (isEmpty(matrix, offsetX, offsetY)) {
          ...
          // Is apply mask.
          if (mask.isApplyMask(mask$1, offsetX, offsetY)) {
            bit ^= 1;
          }
...
function buildMatrix(codewords, version, ecLevel, mask) {
  const matrix = new ByteMatrix.ByteMatrix(version.size);
  // Clear matrix.
  matrix.clear(-1);
